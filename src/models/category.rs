...
impl CategoryTree {
    ...
    pub fn remove(&mut self, id: &str) -> anyhow::Result<()> {
        ...
        self.tree
            .remove_node(&id.to_string(), NodeRemovalStrategy::RemoveNodeAndChildren)?;
        Ok(())
    }
...
mod tests {
    ...
    fn test_category_tree_find_by_path() {
        ...
        assert_eq!(
            tree.find_by_path("餐饮/午餐"),
            Some("cat_lunch".to_string())
        );
        assert_eq!(
            tree.find_by_path("餐饮/午餐/食堂"),
            Some("cat_canteen".to_string())
        );
    ...
    fn test_category_tree_traverse() {
        let mut tree = CategoryTree::new();
        let food = tree.add_root("cat_food", "餐饮").unwrap();
        let _lunch = tree.add_child("cat_lunch", "午餐", &food).unwrap();
        let _dinner = tree.add_child("cat_dinner", "晚餐", &food).unwrap();
