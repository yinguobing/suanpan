...
use crate::output::{print_count, OutputFormat, OutputTable};
...
pub async fn execute(db: &Database, args: ListArgs, output_format: OutputFormat) -> Result<()> {
    // 使用组合查询
    let transactions = db
        .query_transactions(
            args.from.as_deref(),
    ...
    if transactions.is_empty() {
        match output_format {
            ...
            OutputFormat::Human => println!("暂无交易记录"),
        }
        return Ok(());
    }
...
fn generate_output(
    ...
    match format {
        OutputFormatArg::Table => {
            if matches!(output_format, OutputFormat::Machine) {
                // 机器可读格式：字段用 | 分隔
