...
pub async fn execute(db: &Database, args: TrendArgs, output_format: OutputFormat) -> Result<()> {
    // 解析周期类型
    ...
    // 打印趋势表
    print_trend_stats(&stats, &period, from_date, to_date, output_format);

    // 如果按分类展示
    if args.by_category {
        let category_trends = db.get_category_trend_stats(period, from_dt, to_dt).await?;
        print_category_trends(&category_trends, &period, args.show_ids, output_format);
    }
...
fn print_trend_stats(
    ...
    let title = match output_format {
        OutputFormat::Machine => format!("TREND:{}:{}:{}", period_name, from_date, to_date),
        OutputFormat::Human => format!("{}趋势分析 ({} 至 {})", period_name, from_date, to_date),
    };
    ...
    if stats.is_empty() {
        match output_format {
            ...
            OutputFormat::Human => println!("暂无数据"),
        }
        return;
    }
    ...
    match output_format {
        OutputFormat::Machine => {
            println!("时间周期|收入|支出|净收支|笔数");
            for stat in stats {
                let net = stat.income - stat.expense;
                println!(
                    "{}|{}|{}|{}|{}",
                    stat.period_label, stat.income, stat.expense, net, stat.transaction_count
                );
            }
            // 总计行
            ...
            println!(
                "总计|{}|{}|{}|{}",
                total_income, total_expense, total_net, total_count
            );
        }
        OutputFormat::Human => {
    ...
    match output_format {
        OutputFormat::Machine => {
            println!("平均值: 收入:{}|支出:{}", avg_income, avg_expense);
        }
        OutputFormat::Human => {
            println!("平均值: 收入 ¥{} | 支出 ¥{}", avg_income, avg_expense);
        }
...
fn print_category_trends(
    ...
    print_empty_line();
    let title = match output_format {
        OutputFormat::Machine => format!("CATEGORY_TREND:{}", period_name),
        OutputFormat::Human => format!("分类{}度支出趋势", period_name),
    };
    print_title(&title, output_format);
    print_empty_line();
    ...
    if trends.is_empty() {
        match output_format {
            OutputFormat::Machine => println!("NO_DATA"),
            OutputFormat::Human => println!("暂无分类数据"),
        }
        return;
    }
    ...
    match output_format {
        OutputFormat::Machine => {
            // 打印表头
            let mut header = vec!["分类".to_string()];
        ...
        OutputFormat::Human => {
            ...
            table.set_header(headers.iter().map(|s| s.as_str()).collect());
